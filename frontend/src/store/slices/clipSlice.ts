...
export function clipVideo(params: ActionParams) {
  return async () => {
    const {
      successCallback = () => {},
      errorCallback = () => {},
...
export function getAllClipVideos() {
  return async () => {
    dispatch(slice.actions.startLoading());

    const options: APIParams = {
      method: "GET",
      endpoint: PATH.clip.get,
      payload: {},
      isToken: false,
    };
    ...
    try {
      ...
      dispatch(slice.actions.getAllClipVideos(response.data));
    } catch (error) {
      console.log(error);
    } finally {
      dispatch(slice.actions.stopLoading());
...
export function deleteClip(params: ActionParams) {
  return async () => {
    const {
      successCallback = () => {},
      errorCallback = () => {},
